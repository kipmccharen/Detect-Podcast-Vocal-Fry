...
def options(**kwargs) -> Options:
    ...
    parser.add_argument( #NEW TESTLINE
        "--mostrecent",  #NEW TESTLINE
        default=kwargs.get("mostrecent", ""), #NEW TESTLINE
        help="override newer than date, retrieve most recent n episodes", #NEW TESTLINE
    ) #NEW TESTLINE
...
def getpodcast(podcasts: dict, args: Options) -> None:
    ...
    if args.keywords:
        for item in (
            ...
            ("mostrecent", "most recent n episodes") #NEW TESTLINE
        ):
            print("{%s} = %s" % item)
        return
    ...
    if args.mostrecent: #NEW TESTLINE
        fromdate = datetime.datetime.strptime(args.date_from, "%Y-%m-%d")
    else: #NEW TESTLINE
        fromdate = datetime.datetime.strptime("1970-01-01", "%Y-%m-%d") #NEW TESTLINE
...
def process_podcast(pod: str, url: str, headers: dict, fromdate, args: Options):
    ...
    for num, item in enumerate(podcast.items):
        go = True #NEW TESTLINE
        if args.mostrecent: #NEW TESTLINE
            if num + 1 > args.mostrecent: #NEW TESTLINE
                go = False  #NEW TESTLINE
        if go: #NEW TESTLINE
            try:
